...
function endProgram() {
    ...
    fs.writeFile("./dist/team.html", htmlContent, (err) =>
        err
            ? console.log(err)
            : console.log("Success! You've created your team. Check team.html!")
    );
}
...
function generateHTML(teamMembers) {
    ...
    teamMembers.forEach((emp) => {
        ...
        if (emp.getRole() === "Manager") {
            card = `<div class="bg-gray-100 rounded-md shadow-xl" id="managerCard">
            ...
            <div class="p-10 grid-rows-3 grid-col-1 gap-y-4 ">
                <div class="p-4 mb-2 bg-white rounded-md">
                    <p>ID: ${emp.id}</p>
                </div>    
                <div class="p-4 mb-2 bg-white rounded-md">
                    <p class="inline">Email: </p><a class="text-blue-500" href="mailto:${emp.email}">${emp.email}</a>
                </div>
                <div class="p-4 mb-2 bg-white rounded-md">
                    <p>Office No.: ${emp.getOfficeNumber()}</p>                    
                </div>      
            </div>
        </div>`;
        } else if (emp.getRole() === "Engineer") {
            card = `<div class="bg-gray-100 rounded-md shadow-xl" id="engineerCard">
            <header class="bg-red-400 rounded-t-md pt-3 pb-2 px-8 text-xl text-white font-bold">${emp.name}</header>
            ...
            <div class="p-10 grid-rows-3 grid-col-1 gap-y-4 ">
                ...
                <div class="p-4 mb-2 bg-white rounded-md">
                    <p class="inline">Github: </p><a class="inline text-blue-500"
                        href="https://github.com/${emp.github}">${emp.getGithub()}</a>
                </div>
            </div>
        </div>`;
        } else {
            card = `<div class="bg-gray-100 rounded-md shadow-xl" id="internCard">
            <header class="bg-red-400 rounded-t-md pt-3 pb-2 px-8 text-xl text-white font-bold">${emp.name}</header>
